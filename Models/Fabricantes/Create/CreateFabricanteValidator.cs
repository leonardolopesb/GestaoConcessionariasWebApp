    ...
    public class CreateFabricanteValidator : AbstractValidator<CreateFabricanteDto>
    {
        public CreateFabricanteValidator()
        {
            ...
            RuleFor(x => x.AnoFundacao)
                ...
                .WithMessage($"O ano de fundação deve ser menor do que o ano de {DateTime.UtcNow.Year}.");

            RuleFor(x => x.Website)
                .NotEmpty().WithMessage("O website é obrigatório.")
                .Must(IsValidUrl).WithMessage("Website inválido (URL).")
